...
```{r}
bikes_september <- read_csv(here("data_raw","bikes.zip"))%>% clean_names() # bike share data
bikes_lanes <- read_csv(here("data_raw", "Bicycle_Lanes.zip")) %>% clean_names() # bike lane data
bikes_crashes <- read_csv(here("data_raw", "Crashes_in_DC.zip")) %>% clean_names() # bike accident data
```
...
## Breaks down the different type of bike lanes

```{r}
## contraflow bikelanes"
df4 = bikes_lanes |> filter(bikelane_contraflow != "NA")

## seperate the desired column
df4a <- df4[ -c(1:7,9:26) ]

## conventional bikelanes 
df5 = bikes_lanes |> filter(bikelane_conventional != "NA")

## seperate the desired column
df5a <- df5[ -c(1:8,10:26) ]

## dual protected bikelanes
df6 = bikes_lanes |> filter(bikelane_dual_protected != "NA")

## seperate the desired column
df6a <- df6[ -c(1:9,11:26) ]

## dual buffered bikelanes 
df7 = bikes_lanes |> filter(bikelane_dual_buffered != "NA")

## seperate the desired column
df7a <- df7[ -c(1:10,12:26) ]

## protected bikelanes
df8 = bikes_lanes |> filter(bikelane_protected != "NA")

## seperate the desired column
df8a <- df8[ -c(1:11,13:26) ]

## buffered bikelanes 
df9 = bikes_lanes |> filter(bikelane_buffered != "NA")
...
```{r}
## filter out any accidents before jan 1st 2019
bikes_crashesgeo2 = bikes_crashesgeo |> filter(reportdate >= "2019/01/01 05:00:00+00")

## major bicyclist injuries
majorinjuries = bikes_crashesgeo2 |> filter(majorinjuries_bicyclist>=1)

minorinjuries = bikes_crashesgeo2 |> filter(minorinjuries_bicyclist>=1,objectid != 223065960,objectid != 223063821)

fatalinjuries = bikes_crashesgeo2 |> filter(fatal_bicyclist>=1)

minorinjuries %>%
  tm_shape()+
  tm_dots("minorinjuries_bicyclist")

fatalinjuries %>%
  tm_shape()+
  tm_dots("fatal_bicyclist")

majorinjuries %>%
  tm_shape()+
  tm_dots("majorinjuries_bicyclist")
```
