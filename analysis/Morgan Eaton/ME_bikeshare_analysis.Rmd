...
```{r}
bikes <- read_csv(here("data_raw","bikes.zip"))%>% clean_names() # bike share data
lanes <- read_csv(here("data_raw", "Bicycle_Lanes.zip")) %>% clean_names() # bike lane data
crashes <- read_csv(here("data_raw", "Crashes_in_DC.zip")) %>% clean_names() # bike accident data
```
