...
## Investigating a Relationship Between Ridership and Crash Data 

Turning the bike ridership data into a sf object class to allow for plotting on tmap. We will only being looking at the start latitude and longitude of each ride.

```{r}
class(bikes_september)

sf_bikes_september <- bikes_september %>%
  st_as_sf(coords = c("start_lat","start_lng"), crs = 4326) %>%
  st_cast("POINT")

class(sf_bikes_september)

```
Using a smaller set of data for analysis:
