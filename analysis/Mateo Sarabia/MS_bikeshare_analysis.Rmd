...
```{r}
 bikes_lanesgeo2 <- bikes_lanesgeo[ -c(1:7,14:27) ]

class(bikes_lanesgeo2)
bikes_lanesgeo2$geometry[1000]
plot(bikes_lanesgeo2)#+geom_sf(data =, size = .1, color = "black")

```
```{r}
 bikes_crashesgeo2 <- bikes_crashesgeo[ -c(1:11,25:58) ]
#set_crs(longitude,latitude)
class(bikes_crashesgeo2)
bikes_crashesgeo2$geometry[1000]
bikes_crashesgeo2$geometry[1000]$POLYGON
plot(bikes_crashesgeo2)
max.plot = 24
```
