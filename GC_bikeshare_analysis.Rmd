...
summary_crashes <- list(
  ...
  bikelane_contraflow = table(crashes_lanes$bikelane_contraflow)
)
...
ggplot(summary_crashes_df, aes(x = lane_type, y = count, fill = adjacent_lane_type)) +
  ...
  labs(x = "Type of bikelane", y = "Number of Crashes", fill = "Type of adjacent lane")
```
...
bikes$start_station_id <- as.character(bikes$start_station_id)
bikes$end_station_id <- as.character(bikes$end_station_id)

lanes_long <- lanes %>%
  gather(key = "lane_type", value = "presence", bikelane_parkinglane_adjacent:bikelane_buffered) %>%
  filter(presence == "Yes")
# Join the bikes and lanes_long datasets based on the start_station_id and routeid
bikes_start_lanes <- left_join(bikes, lanes_long, by = c("start_station_id" = "routeid"))

# Join the bikes and lanes_long datasets based on the end_station_id and routeid
bikes_end_lanes <- left_join(bikes, lanes_long, by = c("end_station_id" = "routeid"))

# Combine the two joined datasets
bikes_lanes <- bind_rows(bikes_start_lanes, bikes_end_lanes)

# Calculate the total ridership for each type of bike lane
ridership_by_lanetype <- bikes_lanes %>%
  filter(!is.na(lane_type)) %>%
  group_by(lane_type) %>%
  summarise(ridership = n(), .groups = "drop")
`
# Plot the ridership by bike lane type
ggplot(ridership_by_lanetype, aes(x = lane_type, y = ridership)) +
  geom_bar(stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Type of Bike Lane", y = "Total Ridership")
```
